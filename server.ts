...
const env = {
  ...
  liveOpenMeteoEnabled: process.env.SAFEFLOW_ENABLE_LIVE_OPEN_METEO === "true",
  osrmEnabled: process.env.SAFEFLOW_DISABLE_OSRM !== "true"
};
const typhoonEnabled = process.env.SAFEFLOW_ENABLE_TYPHOON === "true" && Boolean(env.typhoonApiKey);
const liveGoogleEnabled = process.env.SAFEFLOW_ENABLE_LIVE_GOOGLE === "true" && Boolean(env.googleApiKey);
...
createServer(async (req, res) => {
  ...
  console.log(`SafeFlow server listening on http://127.0.0.1:${port}`);
});
